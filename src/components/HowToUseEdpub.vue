      ...
      <div class="how_to_use_edpub">
        <div class="mx-5">
          <div class="row pt-5 justify-content-center">
            <h1 class="display-4" v-if="how_to_use_edpub.heading">{{ how_to_use_edpub.heading }}
              <hr>
            </h1>
            <template v-for="(value, index) in how_to_use_edpub.paragraphs" :key="index">
              <h1 class="display-4" id="how"
                v-if="value.heading && value.heading == 'How to Publish with Earthdata Pub'">
                {{ value.heading }}
                <hr>
              </h1>
              <h1 class="display-4" id="scope"
                v-else-if="value.heading && value.heading === 'Data Scope and Acceptance Policy'">{{ value.heading }}
                <hr>
              </h1>
              <span class="display-4 main-width sections"
                v-else-if="value.heading && value.heading.indexOf(':') !== -1"><b>{{ value.heading }}</b></span>
              <h5 v-else-if="value.heading"><b>{{ value.heading }}</b>
                <hr>
              </h5>
